...
<script type="text/javascript" language="javascript">
	function confirmDelete() {
    	return confirm("Are you sure you wish to checkout this guest?");
	} 
</script>
<body>
		...
		<?php
			...
			if(isset($resid)) {
				...
				while($resinfo=mysql_fetch_array($result)) {
					...
					SET ROOM_STATUS=0 WHERE ROOM_ID='".$resinfo['ROOM_ID']."'") or die("Checkout failed");
					$delassign=mysql_query("DELETE FROM ROOM_ASSIGNMENT
					WHERE ROOM_ASSIGNMENT_ID='".$resinfo['ROOM_ASSIGNMENT_ID']."'") or die("Checkout failed");
					
				}
				$delassign=mysql_query("DELETE FROM RESERVATION
				WHERE RESERVATION_ID='".$resid."'") or die("Checkout failed");
				print "Guest successfully checked out.";
				print "<br><a href='checkOut.php'> Back </a>";
			}
			else {
				//pagination
				$result2=mysql_query("SELECT * from RESERVATION,ROOM_ASSIGNMENT WHERE RESERVATION.RESERVATION_ID=ROOM_ASSIGNMENT.RESERVATION_ID AND ROOM_ASSIGNMENT.ROOM_ASSIGNMENT_STATUS=3");
				...
				if((!$page) || (is_numeric($page) == false) || ($page < 0) || ($page > $totalitems)) {
				 ...
				 $result2=mysql_query("SELECT * from RESERVATION,ROOM_ASSIGNMENT WHERE RESERVATION.RESERVATION_ID=ROOM_ASSIGNMENT.RESERVATION_ID AND ROOM_ASSIGNMENT.ROOM_ASSIGNMENT_STATUS=3 LIMIT $set_limit, $limit");
		
				echo "<table width= '600' height='100' BORDER=1 RULES=ROWS FRAME=BOX align='left'>";

				echo "<tr><td colspan=7><h3>Processed Reservation Requests</h3></td></tr>";
				...
				while($row2=mysql_fetch_array($result2)) {
					...
					echo "<td><a href='checkOut.php?id=".$row2['RESERVATION_ID']."' onClick='return confirmDelete();'><img src='../images/door.jpg'></img></a></td>";
					echo "</tr>";
				}
