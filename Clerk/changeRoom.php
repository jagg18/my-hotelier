			...
			if(isset($_POST['submit'])) {
				...
				$date_diff = round(((strtotime($checkout) - strtotime($checkin)) / 86400)); 
							...
							foreach($roomid as $rooms) {
								...
								$assignment=mysql_query("SELECT * FROM RESERVATION,ROOM_ASSIGNMENT WHERE RESERVATION.RESERVATION_ID='".$resid."' AND ROOM_ASSIGNMENT.RESERVATION_ID=RESERVATION.RESERVATION_ID");
								$ass=mysql_fetch_array($assignment);
				
								//set 'from' room to vacant
								//$updateroom=mysql_query("UPDATE ROOM SET ROOM_STATUS='0' WHERE ROOM_ID='".$ass['ROOM_ID']."'") or die("Change room failed.");
								
								$result=mysql_query("UPDATE ROOM_ASSIGNMENT
								SET 
								RESERVATION_ID='".$resid."',
								CLERK_ID='".$userid."',
								ROOM_ID='".$rooms."',
								ROOM_ASSIGNMENT_TOTAL_COST='".$cost."',
								ROOM_ASSIGNMENT_STATUS='1'
								WHERE ROOM_ASSIGNMENT_ID='".$ass['ROOM_ASSIGNMENT_ID']."'") or die("Change room failed.");
								//$roomstatus=mysql_query("UPDATE ROOM SET ROOM_STATUS='1' WHERE ROOM_ID='".$roomid."'") or die("Check-in failed.");
								//$reservationstatus=mysql_query("UPDATE RESERVATION SET ROOM_ASSIGNMENT_ID='".$ass['ROOM_ASSIGNMENT_ID']."' WHERE RESERVATION_ID='".$resid."'") or die("Change room failed.");
							}
								print "You have successfully re-assigned the reservation.<br>";
								print "<br><a href='viewProcessedReservations.php'> Back </a>";
							}
							else{
